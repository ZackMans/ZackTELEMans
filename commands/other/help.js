...
cmd: ['help'],
ignored: true,
async handler(m, { ctx, prefix, q, args }) {
if (!q) return await require('./menu').handler(m, { ctx, prefix, q, args })
...
if (cmd == null) return ctx.reply('Command not found>3')
helpt = '*Helper*\n'
...
helpt += '\n*Command Description*\n'
helpt += `${shp} ${cmd.desc}`
ctx.reply(helpt, { parse_mode: "Markdown" })
}
}